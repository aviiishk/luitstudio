...
export default async function BlogPreview() {
  ...
  return (
    <section className="relative px-5 sm:px-8 py-20 border-t border-gray-100">
      <div className="max-w-6xl mx-auto">

        {/* Header */}
        <div className="flex items-end justify-between mb-10">
          <div>
            <p className="font-body text-[11px] tracking-[0.28em] uppercase text-gray-400 mb-3">
              From the blog
            </p>
            <h2 className="font-heading text-[28px] sm:text-[38px] font-black text-gray-900 leading-tight">
              Latest{" "}
          ...
          <Link
            href="/blog"
            className="hidden sm:flex items-center gap-1.5 text-sm font-body text-gray-400 hover:text-gray-900 transition-colors duration-200 group shrink-0"
          >
            View all posts
            ...
            <Link key={post.id} href={`/blog/${post.slug}`} className="group flex flex-col">
              <article className="flex flex-col h-full rounded-2xl border border-gray-200 bg-white hover:border-gray-300 shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
                ...
                <div className="flex flex-col flex-1 p-5">
                  ...
                  <h3 className="font-heading font-bold text-gray-900 text-[15px] leading-snug mb-2 group-hover:text-[#EC4899] transition-colors duration-200">
                    {post.title}
                  </h3>
        ...
        <div className="sm:hidden mt-7 text-center">
          <Link
            href="/blog"
            className="inline-flex items-center gap-1.5 text-sm font-body text-gray-400 hover:text-gray-900 transition-colors duration-200 group"
          >
            View all posts
            <ArrowRight size={14} className="group-hover:translate-x-0.5 transition-transform duration-200" />
          </Link>
